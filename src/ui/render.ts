...
export function render(): void {
  ...
  const matchOpen = state.current.match !== null;
  const consoleSetting = matchOpen
    ? ""
    : `<label class="console-setting"><input id="require-physical-confirmations" type="checkbox" ${state.current.requirePhysicalConfirmations ? "checked" : ""}> Require manual physical confirmations</label>`;
  const systemCheckPanel = matchOpen
    ? ""
    : `<details class="panel readiness-panel" open><summary><span><span class="eyebrow">System check</span><strong>Readiness</strong></span><span class="readiness-badge" data-state="${blocked ? "blocked" : "ready"}">${blocked ? "Checks required" : "Storage ready"}</span></summary><dl class="status-grid" aria-live="polite"><div class="status-card" data-status="${readiness.network}"><dt>Network</dt><dd>${statusLabel(readiness.network)}</dd><p>${readiness.network === "online" ? "A network connection is available." : "No network connection. The cached shell can still work."}</p></div><div class="status-card" data-status="${readiness.serviceWorker}"><dt>Service worker</dt><dd>${statusLabel(readiness.serviceWorker)}</dd><p>${readiness.serviceWorker === "controlled" ? "This page uses the installed shell." : "Reload after installation so the service worker can control this page."}</p></div><div class="status-card" data-status="${readiness.offline}"><dt>Offline shell</dt><dd>${statusLabel(readiness.offline)}</dd><p>${readiness.offline === "ready" ? "The required app shell is cached." : "The app shell is not ready for an offline launch yet."}</p></div><div class="status-card" data-status="${readiness.canonicalStorage}"><dt>Canonical storage</dt><dd>${statusLabel(readiness.canonicalStorage)}</dd><p>${state.current.storageDetail}</p></div></dl></details>`;
  appRoot.innerHTML = `<main class="shell" ${rulesUi.current.open ? "inert" : ""}><header class="hero"><div class="hero-heading"><div><p class="eyebrow">BOTTLEBOUND</p><h1>Referee Console</h1></div><button id="open-rules" class="secondary-action" type="button">Rules</button></div><p class="lede">Run one reliable initiative order, even after an offline restart.</p>${consoleSetting}</header>${systemCheckPanel}${matchPanel()}</main>${rulesModal()}`;
  appRoot
